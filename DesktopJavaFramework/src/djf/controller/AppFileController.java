...
import static djf.settings.AppPropertyType.EXPORT_WORK_MESSAGE;
...
import static djf.settings.AppPropertyType.CHANGETIME_WORK_MESSAGE;
import static djf.settings.AppPropertyType.CHANGETIME_WORK_TITLE;
...
public class AppFileController {
   ...
   public void handleLoadRequest() {
	...
	PropertiesManager props = PropertiesManager.getPropertiesManager();
      ...
      /**
     * this function was used to handle the save as request 
     */
 ...
 /**
  * this function used to export file
  ...
  */
    private void exportWork(File selectedFile) throws IOException{
     ...
     //   String targetFile=selectedFile.getPath().replaceAll("\\\\"+selectedFile.getName(),"\\\\public_html\\\\");
       String targetFile=selectedFile.getPath().substring(0,selectedFile.getPath().length()-selectedFile.getName().length())+"public_html\\";
        ...
        for(File f:sonfile){
            if (f.isDirectory()){ 
                exportdir(sourceFile+File.separator+f.getName(), targetFile+File.separator+f.getName()); 
            }
           else {
                exportfile(f,new File(targetFile+f.getName()));
            }
       }
    ...
    /**
     * this function is part of export work, used to export file
     * @param sourceFile the file that user want to export
     * @param targetFile the file the user want to export to 
     * @throws IOException 
     */

    public void exportfile (File sourceFile,File targetFile) throws IOException{
       ...
       byte[] by =new byte[1024*10];
        int length=inputbuffer.read(by);
        while(( length)!=-1){
            outputbuffer.write(by,0,length);
            length=inputbuffer.read(by);
        }
        outputbuffer.flush();
        inputbuffer.close();
        outputbuffer.close();
    ...
    /**
     * this function was used to export dir
     * @param sourcedir  the dir you want to export
     * @param targetdir  the dir you want to export to
     * @throws IOException 
     */
   
    public void exportdir(String sourcedir,String targetdir) throws IOException{
        File totaltargetdir=new File(targetdir);   
        boolean temp=totaltargetdir.mkdirs();
        File totalsourcedir=new File(sourcedir);
           File[] sonfile =totalsourcedir.listFiles();
           for(File f:sonfile){
               if(f.isDirectory()){      
        
                   exportdir(sourcedir+File.separator+f.getName(), targetdir+File.separator+f.getName());
               }
               else{
                   exportfile(f, new File(new File(targetdir).getAbsolutePath()+File.separator+f.getName()));
               }
           }
